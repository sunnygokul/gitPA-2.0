...
async function downloadZip() {
  ...
  try {
    ...
    // Add each test file
    props.testCases.forEach((test, idx) => {
      const fileName = test.name.includes('.') ? test.name : `test_${idx + 1}.ts`;
      testsFolder?.file(fileName, test.code);
    });
    
    // Add README
    const readme = `# Generated Test Suite
    
Generated ${props.testCases.length} test${props.testCases.length !== 1 ? 's' : ''}

## Test Files
${props.testCases.map((t, i) => `${i + 1}. ${t.name}`).join('\\n')}

## Usage
1. Install dependencies: \`npm install\` or \`pnpm install\`
2. Run tests: \`npm test\` or \`pnpm test\`

Generated by gitPA AI`;
    ...
    alert('Failed to create ZIP file. Please try copying individual tests.');
  }
}
</script>
